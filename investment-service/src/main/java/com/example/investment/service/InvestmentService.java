...
public class InvestmentService {
    ...
    public Investment findVerifiedInvestmentTransactionWithID(Long id){
        return investmentRepository.findById(id)
                .orElseThrow(() ->new InvestmentNotFoundException(String.format("Failed to find expenses with category %d", id)));
    ...
    public void removeInvestment(long id){
        if(!investmentRepository.existsById(id)){
            throw new InvestmentNotFoundException(String.format("Failed to find expenses with category %d", id));
        }
        else{
